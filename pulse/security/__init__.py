...
SECRET_PATTERNS = [
    ...
    (
        re.compile(
            r'(?i)(secret[_-]?key|secretkey)\s*[:=]\s*["\']?([a-zA-Z0-9_\-]{16,64})["\']?'
        ),
        r"\1=***",
    ),
    (
        re.compile(
            r'(?i)(access[_-]?token|auth[_-]?token)\s*[:=]\s*["\']?([a-zA-Z0-9_\-]{16,64})["\']?'
        ),
        r"\1=***",
    ),
    # Bearer tokens
    ...
    (
        re.compile(
            r"-----BEGIN (RSA |EC |DSA |OPENSSH |PRIVATE )?PRIVATE KEY-----.*?-----END",
            re.DOTALL,
        ),
        r"[PRIVATE KEY REDACTED]",
    ),
    # Generic hex/hash tokens (32+ hex chars)
    (
        re.compile(r"\b[a-f0-9]{32,}\b", re.IGNORECASE),
        lambda m: m.group(0)[:4] + "***" if len(m.group(0)) > 16 else m.group(0),
    ),
    # Password in URL
    (re.compile(r"(?i)://[^:\s]+:([^@]+)@"), r"://***:***@"),
]
...
def create_checkpoint(
    path: Path, label: str = "", max_snapshots: int = 50
) -> Optional[Path]:
...
def list_checkpoints(path: Optional[Path] = None) -> list[dict]:
    ...
    results = []
    if path is not None:
        content_hash = hashlib.sha256(str(Path(path).resolve()).encode()).hexdigest()[
            :12
        ]
