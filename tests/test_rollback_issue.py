...
class TestIssue2Rollback:
    ...
    def test_rollback_restores_known_good_body_and_version(self, tmp_path):
        ...
        baseline_dir = _make_skill(settings.skills_dir, "rollback-demo", "1.0.0", "KNOWN_GOOD_BODY")
        ...
        registry.update_status("rollback-demo", "promoted", metrics={"success_rate": 1.0})
        ...
        assert result.decision == "rollback", f"Expected rollback, got {result.decision}"
        ...
        assert after_apply.version == "1.0.0", f"Expected version 1.0.0, got {after_apply.version}"
        ...
        assert rediscovered.version == "1.0.0", f"Expected 1.0.0 after rediscover, got {rediscovered.version}"
    ...
    def test_rollback_records_baseline_in_eval(self, tmp_path):
        ...
        storage.record_eval(
            run_id="baseline", skill_id=f"{rec.name}@1.0.0",
            baseline_id=None, decision="promote", metrics={"success_rate": 1.0},
        )

        # Create rollback result with baseline
        baseline = SkillRecord(
            id="baseline", name="test-skill", path=rec.path,
            version="1.0.0", status="promoted", metrics={"success_rate": 1.0},
        )
        result = EvalResult(
            skill_name="test-skill", runs=3, success_rate=0.33,
            avg_tokens=10, avg_steps=1, baseline_success_rate=1.0,
            delta_success=-0.67, decision="rollback", reason="regressed",
        ...
        assert latest.get("baseline_id") is not None or "baseline" in str(latest.get("metrics", {})).lower()
    ...
    def test_versioning_rollback_with_no_snapshot_falls_back_to_disk(self, tmp_path):
        ...
        storage.save_skill_version(
            skill_name="no-snapshot", version="0.9.0",
            path=str(skill_dir), status="promoted",
            metrics={}, content_snapshot=None,
        )
